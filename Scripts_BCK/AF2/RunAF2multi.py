...
parser.add_argument("input_files", nargs='+', help="List of input .a3m files")
...
#parser.add_argument("--plddt_dir", help="Directory to write the plddt per residue")
...
def write_results(result, pdb_out_path): # plddt_out_path):
  plddt = float(result['pLDDT'])
  ptm = float(result["pTMscore"])
  print('plddt: %.3f' % plddt)
  print('ptm  : %.3f' % ptm)
  ### Gerard/Adam 24/01/2025
  ### print(f"plddt x res: {result['plddt']}")
  #with open(plddt_out_path, 'w') as file:
    #for value in result['plddt']:
        #file.write(f"{value}\n")
    #file.write(np.array_str(['plddt']))
...
for a3m_file in args.input_files:
    # Call the main code block with the current .a3m file as input
    # Construct the output path for the resulting .pdb file
    output_pdb_path = os.path.join(args.output_dir, os.path.splitext(os.path.basename(a3m_file))[0] + '.pdb')
    ### Gerard
    #output_plddt_path = os.path.join(args.plddt_dir, os.path.splitext(os.path.basename(a3m_file))[0] + '.txt')

    # Call the main code block with the current .a3m file as input
    runner = make_model_runner(model_name, args.recycles)
    name = os.path.basename(a3m_file).replace('.a3m', '')
    features = make_processed_feature_dict(runner, a3m_file, name=name, seed=args.seed)
    result = parse_results(runner.predict(features, random_seed=args.seed), features)

    # Write the result to the output .pdb file
    write_results(result, output_pdb_path) #, output_plddt_path)
...
#features = make_processed_feature_dict(runner, args.input_msa, name=name, seed=args.se)
#result = parse_results(runner.predict(features, random_seed=args.seed), features)
