...
for line in lines[1:]:
  columns = line.split()
  apc[int(columns[0])-1][int(columns[1])-1] = float(columns[3])
  apc[int(columns[1])-1][int(columns[0])-1] = apc[int(columns[0])-1][int(columns[1])-1]
...
for line in lines[1:]:
    columns = line.split()
    zscore[int(columns[0])-1][int(columns[1])-1] = zscore_norm[idx]
    zscore[int(columns[1])-1][int(columns[0])-1] = zscore[int(columns[0])-1][int(columns[1])-1]
    idx = idx + 1
